...
function usage() {
  ...
  console.error(
    'Usage: node app.js <entry type> <participant name> <entry name> <entry id>',
  );
  console.error(
    "Entry type can be: 'participant', 'data-resource', 'service-offering' or 'data-space-connector'",
  );
...
const claimsTemplateFile = path.resolve(
  path.join(claimsPath, `compliant-${entryType}-template.json`),
);
...
function issueComplianceCredential() {
  ...
  const compliantClaimsFilePath = path.resolve(
    path.join(
      finalClaimsPath,
      (entryName ? entryName : participantName) +
        `-${entryType}-compliant.json`,
    ),
  );
  writeFileSync(
    compliantClaimsFilePath,
  ...
  const finalComplianceCredentialPath =
    (entryName ? entryName : participantName) + `-compliant-${entryType}.json`;
  renameFile(stderr.toString(), finalComplianceCredentialPath);
  console.log(
    'Compliance credential created successfully: ',
    finalComplianceCredentialPath,
  );
}
...
function createSignedCredential(credentialPath, identityData, identityFile) {
  ...
  const signedCredentialPath = path.resolve(
    path.join('../docs/public-credentials', fileName),
  );
