...
export class WebinarDataSpaceConnectorApp implements IDataSpaceConnectorApp {
	...
	constructor(options: IWebinarAppConstructorOptions) {
		...
		DataTypeHandlerFactory.register("https://vocabulary.uncefact.org/Document", () => ({
			context: "https://vocabulary.uncefact.org/",
			type: "Document",
			defaultValue: {},
			jsonSchema: async () => ({
	...
	public async handleActivity<T>(activity: IActivity): Promise<T> {
		...
		try {
			...
			switch (activityType) {
				...
				case "Create": {
					const vertexIdObject = await this.createItem(activity.object, userIdentity, this._nodeIdentity);
					await this._loggingService?.log({
						level: "info",
						source: this.CLASS_NAME,
				...
				case "Add": {
					...
					const objectVertexId = await this._aigComponent.create(aig, userIdentity, this._nodeIdentity);
					await this._loggingService?.log({
						level: "info",
						source: this.CLASS_NAME,
					...
					if (results.itemListElement.length === 0) {
						targetVertexId = await this.createItem(activity.target, userIdentity, this._nodeIdentity);
						await this._loggingService?.log({
							level: "info",
							source: this.CLASS_NAME,
					...
					await this._aigComponent.update(updateObject, userIdentity, this._nodeIdentity);
					...
					if (!Is.undefined(consignmentData)) {
						...
						const destCountry = consignmentData?.destinationCountry as { countryId: string };
						const destinationCountry = destCountry?.countryId;
						const documentTypeCode = activity.object.documentTypeCode as string;

						if (
							commodity === "09011101" &&
							destinationCountry && destinationCountry.includes("#GB") &&
							documentTypeCode.includes("#853")
						) {
							// Find the FSA endpoint to notify such a Participant
							const dsConnectors = await this._fedCatalogue.queryDataSpaceConnectors(
								undefined,
	...
	private async createItem(
		...
		const vertexIdObject = await this._aigComponent.create(aig, userIdentity, nodeIdentity);
